...
<template>
  <form @submit="submit" class="grid gap-4" novalidate>
    <label class="grid gap-1">
      ...
      <input v-model="form.name" required type="text" :disabled="status === 'ok'"
        class="border-b border-brand-line bg-transparent py-2 text-brand-fg outline-none focus:border-brand-accent disabled:opacity-50" />
    </label>

    <label class="grid gap-1">
      <span class="font-body text-eyebrow uppercase text-brand-muted">Email</span>
      <input v-model="form.email" required type="email" :disabled="status === 'ok'"
        class="border-b border-brand-line bg-transparent py-2 text-brand-fg outline-none focus:border-brand-accent disabled:opacity-50" />
    </label>

    <label class="grid gap-1">
      <span class="font-body text-eyebrow uppercase text-brand-muted">Mensaje</span>
      <textarea v-model="form.message" required rows="4" :disabled="status === 'ok'"
        class="border-b border-brand-line bg-transparent py-2 text-brand-fg outline-none focus:border-brand-accent resize-none disabled:opacity-50" />
    </label>
